...
reposition robots n = do 
  ...
  traceShow (iterations - n) $
    traceShow (printFloor updatedRobots) $ 
...
printFloor :: [Robot] -> Maybe [[Char]]
printFloor robots = do 
  let floor = replicate floorY $ replicate floorX '.'
  let floor' = foldl (\acc ((x, y), _) -> do 
        let row = acc !! y
        let newRow = take x row ++ "#" ++ drop (x + 1) row
        take y acc ++ [newRow] ++ drop (y + 1) acc
        ) floor robots
  mapM_ traceShowM floor'
  Nothing
